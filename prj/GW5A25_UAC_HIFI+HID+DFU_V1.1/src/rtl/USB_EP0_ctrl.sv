...
localparam DFU_INTERFACE_NUM     = 8'h03;
...
//DFU
localparam os20_app_des_array_t APP_OS20_DES = build_app_os20_des();
localparam os20_dfu_des_array_t DFU_OS20_DES = build_dfu_os20_des();
        ...
        if (setup_active) begin
            if (usb_rxval) begin
                case (stage)
                    ...
                    // ----------------------------------------------------------------
                            {GET_VENDOR  , VENDOR_CODE   , 16'b?    , {8'h00, GET_DESCRIPTOR_SET}   }               : active_cmd <= CMD_GET_MS_OS20_DES;
            ...
            if (setup_pkt.bmRequestType[7] == 1'b1) begin
                if (usb_txact && (endpt_sel == 4'd0)) begin
                    if (usb_txpop) begin
                        ...
                        // 2. 64 字节切包
                        if (((sub_stage + 1'b1) & 8'h3F) == 8'd0) begin
                            endpt0_send <= 1'b0;
                        end
...
//====  DFU Output
            ...
            CMD_GET_MS_OS20_DES: begin
                    endpt0_dat = i_dfu_mode ? DFU_OS20_DES[sub_stage] : APP_OS20_DES[sub_stage] ; 
            end
...
//只要这是一包音频数据，结束时无论是否校验提前终止，都更新 dop_en
            if (audio_pkt_rxing) begin
                //如果当前已经是 DSD 模式，强制不允许拉高 DoP，防止误判
